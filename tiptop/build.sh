...
cp tiptop.spec rpmbuild/SPECS
cp tiptop-2.3.tar.gz rpmbuild/SOURCES

docker pull $img_tag
docker run  --rm -v $PWD/rpmbuild:$PWD/rpmbuild  --net=host $img_tag\
	rpmbuild --define "dist $dist" --define "_topdir $PWD/rpmbuild" -ba $PWD/rpmbuild/SPECS/tiptop.spec
